...
const CurrencyInput = React.forwardRef<HTMLInputElement, CurrencyInputProps>(
  ({ value: rhfValue, onChange: rhfOnChange, onBlur: rhfOnBlur, name, className, ...rest }, ref) => {
    ...
    const handleNumericFormatValueChange = (values: NumberFormatValues, sourceInfo: SourceInfo) => {
      if (rhfOnChange && sourceInfo.source === 'event') {
        ...
        if (typeof values.floatValue === 'number' && !isNaN(values.floatValue)) {
          ...
          // If floatValue is undefined (empty input) or NaN, treat as null for RHF
          valToPass = null;
        }
    ...
    const valueForNumericFormat = (rhfValue === null || rhfValue === undefined || rhfValue === '')
      ? undefined // Pass undefined to NumericFormat for empty/null/undefined RHF value
      : Number(rhfValue);
    ...
    return (
      <NumericFormat
        ...
        customInput={React.forwardRef((props, inputRef) => (
          // Explicitly set type="tel" here
          <Input {...props} type="tel" ref={inputRef as React.Ref<HTMLInputElement>} className={cn("text-base md:text-sm", className)} />
        ))}
        ...
        fixedDecimalScale
        allowNegative={false}
        // type="text" // This outer type on NumericFormat is less critical than the inner input's type
        autoComplete="off"
      />
    );
  }
